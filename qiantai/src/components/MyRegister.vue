    ...
    <el-dialog title="注册" width="300px" center :visible.sync="isRegister">
      <el-form :model="RegisterUser" :rules="rules" status-icon ref="ruleForm" class="demo-ruleForm">
        <el-form-item prop="role">
          <el-radio v-model='RegisterUser.role' label='大学生'>大学生</el-radio>
<!--<el-radio v-model='RegisterUser.role' label='团队负责人'>团队负责人</el-radio>-->
<!--<el-radio v-model='RegisterUser.role' label='指导教师'>指导教师</el-radio>-->
  ...
  data() {
    ...
    let validateName = (rule, value, callback) => {
      ...
      if (userNameRule.test(value)) {
        //判断数据库中是否已经存在该用户名
		 let bb=this.RegisterUser.role


          if (bb === '大学生') {
              this.$axios
                  .post("/api/front/findyonghuyonghuming", {xuehao: this.RegisterUser.name})
                  .then(res => {
                      // “001”代表用户名不存在，可以注册
                      if (res.data.code == "0") {
    ...
    Register() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        ...
        if (valid) {
            ...
            if (aa === '大学生') {
                this.$axios
                    .post("/api/front/zhuceyonghu", {
                        xuehao: this.RegisterUser.name,
                        mima: this.RegisterUser.pass,
                    })
                    .then(res => {
                        // “001”代表注册成功，其他的均为失败
                        if (res.data.code === "0") {
