<template>
	<div>
		<el-form :inline="true" :model="formInline" class="user-search">
			<el-form-item label=""><el-input size="small" v-model="formInline.leixing" placeholder="输入类型"></el-input></el-form-item>			
			<el-form-item>
		...
		<el-table size="small" :data="listData" highlight-current-row v-loading="loading" border element-loading-text="拼命加载中" @selection-change="handleSelectionChange">
			...
			<el-table-column align="center" label="操作" min-width="160">
				<template slot-scope="scope">
					...
					<el-button size="mini" type="danger" @click="deleteJingsaileixing(scope.$index, scope.row)" v-if="userInfo.roles[0] == 'guanliyuan' ">删除</el-button>
...
import { jingsaileixingList, jingsaileixingSave, jingsaileixingDelete,jingsaileixingEdit,jingsaileixingDeleteList } from '@/api/jingsaileixing/jingsaileixingApi';
...
export default {
	...
	data() {
		return {
			...
			rules: {
				leixing: [{ required: true, message: '请输入类型', trigger: 'blur' },				],				
			},
	...
	methods: {
		...
		//zxhifuff
		handleShenheList(){
				const jingsaileixings = this.jingsaileixings;
				if(jingsaileixings.length == 0){
					this.$message({
						type: 'error',
						message: '请至少选择一项'
					})
					...
					this.$confirm('确定要批量审核吗?', '信息', {
						...
						jingsaileixings.forEach(item=> {
							let data = {...item, issh: 1}
							jingsaileixingEdit(data).then(res => {})
						})
						this.getdata(this.formInline)
						this.$message({
							type: 'success',
		...
		daochu(){
			axios.get('/jingsaileixing/getExcel', {
				responseType: 'blob',
		...
		submitForm(editData) {
			...
			this.$refs[editData].validate(valid => {
				if (valid) {
					if(this.editForm.id){
						jingsaileixingEdit(this.editForm).then(res => {
						...
						jingsaileixingSave(this.editForm).then(res => {
							this.editFormVisible = false
							this.loading = false
							if (res.code == '0') {
		...
		// 删除公司
		deleteJingsaileixing(index, row) {
			this.$confirm('确定要删除吗?', '信息', {
				confirmButtonText: '确定',
				cancelButtonText: '取消',
				type: 'warning'
			}).then(() => {
				jingsaileixingDelete(row.id).then(res => {
					if (res.code == '0') {
						this.$message({
								type: 'success',
							message: '竞赛类型已删除!'
		...
		handleDeleteList(){
			...
			if(jingsaileixings.length == 0){
				...
				this.$confirm('确定要批量删除吗?', '信息', {
					...
					jingsaileixingDeleteList(jingsaileixings).then(res => {
						if (res.code == '0') {
							this.$message({
								type: 'success',
								message: '批量删除成功!'
