<template>
	<div>
		...
		<el-table size="small" :data="listData" highlight-current-row v-loading="loading" border element-loading-text="拼命加载中" @selection-change="handleSelectionChange">
			...
			<el-table-column align='center' sortable prop='cansaizuopin' label='参赛作品' width='120'><template slot-scope='scope'><el-button @click="download(scope.row)">下载</el-button></template></el-table-column>
			...
			<el-table-column align="center" label="操作" min-width="160">
				<template slot-scope="scope">
					<el-button size='mini' type='primary' @click='dd(scope.$index, scope.row)' v-if="userInfo.roles[0] == 'zhidaojiaoshi'&&scope.row.issh=='已通过'">作品评分</el-button>
					<el-button size='mini' type='primary' @click='hsgshenhe(scope.$index, scope.row)' v-if="userInfo.roles[0] == 'guanliyuan'">审核</el-button>
					<!--liangbuan-->
					<el-button size="mini" type="success" @click="handleEdit(scope.$index, scope.row)" v-if="userInfo.roles[0] == 'guanliyuan'  || userInfo.roles[0] == 'tuanduifuzeren'">编辑</el-button>
					<el-button size="mini" type="danger" @click="deleteBaomingcansai(scope.$index, scope.row)" v-if="userInfo.roles[0] == 'guanliyuan'  || userInfo.roles[0] == 'tuanduifuzeren'">删除</el-button>
...
export default {
	...
	methods: {
		...
		//zxhifuff
		handleShenheList(){
				const baomingcansais = this.baomingcansais;
				if(baomingcansais.length == 0){
					this.$message({
						type: 'error',
						message: '请至少选择一项'
					})
					...
					this.$confirm('确定要批量审核吗?', '信息', {
						...
						baomingcansais.forEach(item=> {
							let data = {...item, issh: 1}
							baomingcansaiEdit(data).then(res => {})
						})
						this.getdata(this.formInline)
						this.$message({
							type: 'success',
		...
		daochu(){
			axios.get('/baomingcansai/getExcel', {
				responseType: 'blob',
		...
		handleDetail(index, row) {const w = window.open("about:blank");w.location.href = 'http://localhost:8080/#/baomingcansaidetail/'+row.id;},
		//liangbuedit
		// 编辑、增加页面保存方法
		submitForm(editData) {
			...
			this.$refs[editData].validate(valid => {
				if (valid) {
					if(this.editForm.id){
						baomingcansaiEdit(this.editForm).then(res => {
		...
		// 删除公司
		deleteBaomingcansai(index, row) {
			this.$confirm('确定要删除吗?', '信息', {
				confirmButtonText: '确定',
				cancelButtonText: '取消',
				type: 'warning'
			}).then(() => {
				baomingcansaiDelete(row.id).then(res => {
					if (res.code == '0') {
						this.$message({
								type: 'success',
							message: '报名参赛已删除!'
		...
		handleDeleteList(){
			...
			if(baomingcansais.length == 0){
				...
				this.$confirm('确定要批量删除吗?', '信息', {
					...
					baomingcansaiDeleteList(baomingcansais).then(res => {
						if (res.code == '0') {
							this.$message({
								type: 'success',
								message: '批量删除成功!'
