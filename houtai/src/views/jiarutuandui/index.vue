		...
		<el-table size="small" :data="listData" highlight-current-row v-loading="loading" border element-loading-text="拼命加载中" @selection-change="handleSelectionChange">
			...
			<el-table-column align="center" label="操作" min-width="160">
				<template slot-scope="scope">
					...
					<!--liangbuan-->
					<el-button size="mini" type="success" @click="dd(scope.$index, scope.row)" v-if="userInfo.roles[0] == 'tuanduifuzeren'  || userInfo.roles[0] == ''">团员</el-button>
					...
					<el-button size="mini" type="danger" @click="deleteJiarutuandui(scope.$index, scope.row)" v-if="userInfo.roles[0] == 'guanliyuan'  || userInfo.roles[0] == ''">删除</el-button>
...
import { jiarutuanduiList, jiarutuanduiSave, jiarutuanduiDelete,jiarutuanduiEdit,jiarutuanduiDeleteList } from '@/api/jiarutuandui/jiarutuanduiApi';
...
export default {
	...
	created() {
		...
		if(this.userInfo.roles[0] == 'daxuesheng'){this.formInline.issh= 0;}
		if(this.userInfo.roles[0] == 'tuanduifuzeren'){this.formInline.issh= 0;}
	...
	methods: {
		...
		//zxhifuff
		handleShenheList(){
				const jiarutuanduis = this.jiarutuanduis;
				if(jiarutuanduis.length == 0){
					this.$message({
						type: 'error',
						message: '请至少选择一项'
					})
					...
					this.$confirm('确定要批量审核吗?', '信息', {
						...
						jiarutuanduis.forEach(item=> {
							let data = {...item, issh: 1}
							jiarutuanduiEdit(data).then(res => {})
						})
						this.getdata(this.formInline)
						this.$message({
							type: 'success',
		...
		handleDetail(index, row) {const w = window.open("about:blank");w.location.href = 'http://localhost:8080/#/jiarutuanduidetail/'+row.id;},
		//liangbuedit
		// 编辑、增加页面保存方法
		submitForm(editData) {
			...
			this.$refs[editData].validate(valid => {
				if (valid) {
					if(this.editForm.id){
						jiarutuanduiEdit(this.editForm).then(res => {
		...
		// 删除公司
		deleteJiarutuandui(index, row) {
			this.$confirm('确定要删除吗?', '信息', {
				confirmButtonText: '确定',
				cancelButtonText: '取消',
				type: 'warning'
			}).then(() => {
				jiarutuanduiDelete(row.id).then(res => {
					if (res.code == '0') {
						this.$message({
								type: 'success',
							message: '加入团队已删除!'
		...
		handleDeleteList(){
			...
			if(jiarutuanduis.length == 0){
				...
				this.$confirm('确定要批量删除吗?', '信息', {
					...
					jiarutuanduiDeleteList(jiarutuanduis).then(res => {
						if (res.code == '0') {
							this.$message({
								type: 'success',
								message: '批量删除成功!'
