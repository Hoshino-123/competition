	...
	<div>
		...
		<el-table size="small" :data="listData" highlight-current-row v-loading="loading" border element-loading-text="拼命加载中" @selection-change="handleSelectionChange">
			...
			<el-table-column align="center" label="操作" min-width="160">
				<template slot-scope="scope">
					...
					<el-button size="mini" type="danger" @click="deleteZhidaojiaoshi(scope.$index, scope.row)" v-if="userInfo.roles[0] == 'guanliyuan' ">删除</el-button>
		...
		<el-dialog :title="title" :visible.sync="editFormVisible" width="30%" @click="closeDialog">
			...
			<table width="100%" border="0"  align="center" cellpadding="3" cellspacing="1" class="table table-striped table-bordered table-hover">
								<tr>
									<td width='11%' height=44>工号：</td><td width='39%'>{{editForm.gonghao}}</td><td  rowspan=6 align=center><el-upload class='avatar-uploader'  :headers='headers' :show-file-list='false'  ><img v-if='editForm.zhaopian' :src='editForm.zhaopian' class='avatar'><i v-else class='el-icon-plus avatar-uploader-icon'></i></el-upload></td></tr><tr><td width='11%' height=44>密码：</td><td width='39%'>{{editForm.mima}}</td></tr><tr><td width='11%' height=44>姓名：</td><td width='39%'>{{editForm.xingming}}</td></tr><tr><td width='11%' height=44>性别：</td><td width='39%'><div v-if="editForm.xingbie==1">男</div><div v-else>女</div></td></tr><tr><td width='11%' height=44>年龄：</td><td width='39%'>{{editForm.nianling}}</td></tr><tr><td width='11%' height=44>手机号码：</td><td width='39%'>{{editForm.shoujihaoma}}</td></tr><t
							</table>
...
import { zhidaojiaoshiList, zhidaojiaoshiSave, zhidaojiaoshiDelete,zhidaojiaoshiEdit,zhidaojiaoshiDeleteList } from '@/api/zhidaojiaoshi/zhidaojiaoshiApi';
...
export default {
	...
	methods: {
		...
		//zxhifuff
		handleShenheList(){
				const zhidaojiaoshis = this.zhidaojiaoshis;
				if(zhidaojiaoshis.length == 0){
					this.$message({
						type: 'error',
						message: '请至少选择一项'
					})
					...
					this.$confirm('确定要批量审核吗?', '信息', {
						...
						zhidaojiaoshis.forEach(item=> {
							let data = {...item, issh: 1}
							zhidaojiaoshiEdit(data).then(res => {})
						})
						this.getdata(this.formInline)
						this.$message({
							type: 'success',
		...
		daochu(){
			axios.get('/zhidaojiaoshi/getExcel', {
				responseType: 'blob',
		...
		// 删除公司
		deleteZhidaojiaoshi(index, row) {
			this.$confirm('确定要删除吗?', '信息', {
				confirmButtonText: '确定',
				cancelButtonText: '取消',
				type: 'warning'
			}).then(() => {
				zhidaojiaoshiDelete(row.id).then(res => {
					if (res.code == '0') {
						this.$message({
								type: 'success',
							message: '指导教师已删除!'
		...
		handleDeleteList(){
			...
			if(zhidaojiaoshis.length == 0){
				...
				this.$confirm('确定要批量删除吗?', '信息', {
					...
					zhidaojiaoshiDeleteList(zhidaojiaoshis).then(res => {
						if (res.code == '0') {
							this.$message({
								type: 'success',
								message: '批量删除成功!'
