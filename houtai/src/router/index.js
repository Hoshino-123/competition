...
const staticRoutes = [
	...
	{path: '/xinwentongzhi/tinymceEditor/:id',name: 'tinymceEditor',component: () => import('@/views/xinwentongzhi/tinymceEditor.vue'),meta: {title: 'message.staticRoutes.tinymceEditor',},},	
];
...
export function daxueshengUser(router, to, next) {
	resetRouter();
	menuApi
		.getMenuDaxuesheng()
		.then(async (res) => {
		...
		.catch(() => {});
}export function tuanduifuzerenUser(router, to, next) {
	resetRouter();
	menuApi
		.getMenuTuanduifuzeren()
		.then(async (res) => {
			// 读取用户信息，获取对应权限进行判断
			store.dispatch('userInfos/setUserInfos');
			store.dispatch('routesList/setRoutesList', setFilterMenuFun(res.data, store.state.userInfos.userInfos.roles));
		...
		.catch(() => {});
}export function zhidaojiaoshiUser(router, to, next) {
	resetRouter();
	menuApi
		.getMenuZhidaojiaoshi()
		.then(async (res) => {
			// 读取用户信息，获取对应权限进行判断
			store.dispatch('userInfos/setUserInfos');
			store.dispatch('routesList/setRoutesList', setFilterMenuFun(res.data, store.state.userInfos.userInfos.roles));
...
export function getRouterList(router, to, next) {
	...
	if (Session.get('userInfo').roles[0] === 'guanliyuan') guanliyuanUser(router, to, next);
	if (Session.get('userInfo').roles[0] === 'daxuesheng') daxueshengUser(router, to, next);if (Session.get('userInfo').roles[0] === 'tuanduifuzeren') tuanduifuzerenUser(router, to, next);if (Session.get('userInfo').roles[0] === 'zhidaojiaoshi') zhidaojiaoshiUser(router, to, next);
}
