...
<script>
	...
	import {daxueshengUpdatePassword} from "@/api/daxuesheng/daxueshengApi";	import {tuanduifuzerenUpdatePassword} from "@/api/tuanduifuzeren/tuanduifuzerenApi";	import {zhidaojiaoshiUpdatePassword} from "@/api/zhidaojiaoshi/zhidaojiaoshiApi";	
				...
				this.$refs[editData].validate(valid => {
					if (valid) {
						if (this.editForm.newPassword === this.editForm.newPassword2) {
							...
							else if (this.userInfo.roles[0] == 'daxuesheng') {
								daxueshengUpdatePassword(this.editForm).then(res => {
									if (res.code == "0") {
										this.$message.success('修改成功，为了安全，系统将自动清空session，请您重新登陆')
										setTimeout(() => {
											Session.clear();
							...
							}else if (this.userInfo.roles[0] == 'tuanduifuzeren') {
								tuanduifuzerenUpdatePassword(this.editForm).then(res => {
									if (res.code == "0") {
										this.$message.success('修改成功，为了安全，系统将自动清空session，请您重新登陆')
										setTimeout(() => {
											Session.clear();
							...
							}else if (this.userInfo.roles[0] == 'zhidaojiaoshi') {
								zhidaojiaoshiUpdatePassword(this.editForm).then(res => {
									if (res.code == "0") {
										this.$message.success('修改成功，为了安全，系统将自动清空session，请您重新登陆')
										setTimeout(() => {
											Session.clear();
